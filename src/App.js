...
function CharacterSprite(props) {
  ...
  const eyes = (function () {
    if (emotion === "happy") {
      return (
        <g>
          <path d="M26 34 q5 -6 10 0" stroke="#1e1b4b" strokeWidth="3" fill="none" strokeLinecap="round" />
          <path d="M44 34 q5 -6 10 0" stroke="#1e1b4b" strokeWidth="3" fill="none" strokeLinecap="round" />
        </g>
      );
    }
    if (emotion === "thinking") {
      return (
        <g>
          <circle cx="31" cy="31" r="4" fill="#1e1b4b" />
          <circle cx="49" cy="31" r="4" fill="#1e1b4b" />
          <circle cx="32.5" cy="29.5" r="1.4" fill="#fff" />
          <circle cx="50.5" cy="29.5" r="1.4" fill="#fff" />
        </g>
      );
    }
    return (
      <g className="tb-blink">
        <circle cx="31" cy="34" r="4.5" fill="#1e1b4b" />
        <circle cx="49" cy="34" r="4.5" fill="#1e1b4b" />
        <circle cx="32.7" cy="32.3" r="1.6" fill="#fff" />
        <circle cx="50.7" cy="32.3" r="1.6" fill="#fff" />
      </g>
    );
  })();
  const mouth = talking
    ? <ellipse cx="40" cy="46" rx="5" ry="4" fill="#1e1b4b" className="tb-talk" />
    : emotion === "happy"
      ? <path d="M33 44 q7 7 14 0" stroke="#1e1b4b" strokeWidth="3" fill="none" strokeLinecap="round" />
      : <path d="M35 46 q5 4 10 0" stroke="#1e1b4b" strokeWidth="2.5" fill="none" strokeLinecap="round" />;
  return (
    <svg width={CHAR_W} height={CHAR_H} viewBox="0 0 80 88" style={{ transform: facing === "left" ? "scaleX(-1)" : "none", display: "block", overflow: "visible" }}>
      <defs>
      ...
      {/* 그림자 */}
      <ellipse cx="40" cy="82" rx={walking ? 20 : 23} ry="4.5" fill="#000" opacity="0.25" className={walking ? "" : "tb-shadow"} />
      <g className={walking ? "tb-walk" : "tb-idle"}>
        {/* 안테나 */}
        <line x1="40" y1="10" x2="40" y2="2" stroke="#818cf8" strokeWidth="2.5" strokeLinecap="round" />
        <circle cx="40" cy="1.5" r="3.5" fill="#fbbf24" className="tb-antenna" />
        {/* 몸통 */}
        <path d="M40 8 C 18 8, 10 26, 10 44 C 10 64, 22 76, 40 76 C 58 76, 70 64, 70 44 C 70 26, 62 8, 40 8 Z" fill="url(#tbBody)" />
        {/* 얼굴판 */}
        <ellipse cx="40" cy="38" rx="22" ry="18" fill="#ffffff" opacity="0.92" />
        {eyes}
        {mouth}
        {/* 볼터치 */}
        <circle cx="22" cy="42" r="3.5" fill="#ec4899" opacity="0.4" />
        <circle cx="58" cy="42" r="3.5" fill="#ec4899" opacity="0.4" />
        {/* 팔: 가리키기 모드면 한쪽 팔을 위로 */}
        {pointing ? (
          <g className="tb-point">
            <line x1="66" y1="40" x2="79" y2="22" stroke="#818cf8" strokeWidth="6" strokeLinecap="round" />
            <circle cx="79" cy="21" r="4.5" fill="#818cf8" />
          </g>
        ) : (
          <g>
            <line x1="13" y1="48" x2="5" y2="56" stroke="#818cf8" strokeWidth="6" strokeLinecap="round" className={walking ? "tb-armL" : ""} />
            <line x1="67" y1="48" x2="75" y2="56" stroke="#818cf8" strokeWidth="6" strokeLinecap="round" className={walking ? "tb-armR" : ""} />
          </g>
        )}
        {/* 발 */}
        <ellipse cx="30" cy="77" rx="7" ry="4" fill="#6366f1" className={walking ? "tb-footL" : ""} />
        <ellipse cx="50" cy="77" rx="7" ry="4" fill="#6366f1" className={walking ? "tb-footR" : ""} />
      </g>
    </svg>
  );
}
